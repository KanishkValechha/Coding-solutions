 ...
 * 
 * A
0-indexed array derived with length n is derived by computing the
bitwise XOR
bitwise XOR (⊕) of adjacent values in a
binary array
...
Return
true
true if such an array exists or
false
false otherwise.
...
        int ans=count(derived.begin(),derived.end(),1);
    }
        return ans%2==0;
};